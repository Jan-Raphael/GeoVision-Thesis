...
export function CaptureLightbox({
  ...
  return (
    ...
    >
      <div
        className="max-h-full w-full max-w-4xl overflow-y-auto rounded-xl bg-white p-5"
        onClick={(event) => {
          event.stopPropagation();
        }}
      >
        <div className="flex items-start justify-between gap-3">
          <h2 className="truncate font-mono text-sm text-slate-700">
            {detail.data?.filename ?? 'Capture'}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        {detail.isPending && <div className="mt-4 h-80 animate-pulse rounded-lg bg-slate-200" />}
        {detail.isError && (
          <div className="mt-4">
