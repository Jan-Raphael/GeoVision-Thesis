...
| Q17 | **Coverage `fail_under` thresholds were set from partial evidence.** | `backend/pyproject.toml`'s `fail_under = 60` is a verified-safe floor from a **unit-only** run (63.10%) — Docker was not available to run `tests/integration/` (needs Postgres/Redis/MinIO) in the same session, so the real combined number CI actually measures (unit + integration together) is unknown but can only be *higher*. `ai/pyproject.toml`'s `fail_under = 85` **is** the full, real number (89.61% measured, no services required). Raise the backend threshold toward the vault's stated ≥80% target once a run against live services confirms the actual combined figure — do not lower either number without a reason recorded here. | you | **open — needs one CI run or one local run with `make up`** |
| Q18 | **The fused progress formula (classifier + YOLO detections + physical change) is undecided.** | [[ADR-Index#ADR-036\|ADR-036]] narrowed the classifier to 4 macro-aligned classes and moved sub-stage resolution to a fusion of classifier class + YOLO detections + frame-to-frame physical change, replacing the old 10-class nominal-percentage table. The team confirmed the *inputs* (2026-08-18 chat) but not the *formula*. [[Progress-Calculation]] §1/§5 and [[Module-08-YOLO-Detection]]'s corroboration section are marked superseded pending this. A concrete starting proposal (not yet agreed): keep §2–§7 of [[Progress-Calculation]] (median → multi-camera fusion → EMA → ratchet) exactly as-is — they operate on whatever `raw_pct` comes out of step 1 regardless of how it's computed — and only replace step 1's `raw_pct = PROGRESS_REFERENCE[class].nominal_pct` with `raw_pct = stage_floor + sub_stage_fraction * 20`, where `stage_floor`/ceiling come from the classifier's macro-stage class (unchanged), and `sub_stage_fraction ∈ [0,1]` is derived per-stage from a checklist of the YOLO elements expected in that stage (e.g. structural: rebar/column only → ~0.25, + beam → ~0.5, + wall/CHB rising → ~0.75, wall enclosure complete → 1.0; finishing: fraction = distinct finishing element types detected ÷ 5). "Physical change" (frame-to-frame difference between consecutive captures from the same device) would corroborate/flag rather than directly set a percentage — e.g. near-zero change for a long time while the classifier holds a stage is normal, but very large change might mean camera movement or a different structure rather than progress, and could suppress that window's contribution instead of trusting it. This needs the team's sign-off (or a different design) before `ai/progress/aggregator.py`, `classes.yaml`, or any evaluation code changes — Progress-Calculation.md is explicitly the thesis's core contribution and the existing algorithm is fully tested with a worked example (§8) that a wrong formula would silently invalidate. | you | **open — blocks Module 07/08/09 rework** |
