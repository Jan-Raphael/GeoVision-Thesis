...
| Q15 | **`storage_backend` and `nonce_cache_backend` have the same test/production asymmetry that hid the startup bug.** | Tests build settings with `storage_backend="local"` and `nonce_cache_backend="memory"`, while the shipped defaults are `minio` and `redis`. Those adapters are therefore constructed by *no test* — exactly the condition that let `RealtimeSubscriber` reach production unbuilt (see [[ADR-Index#ADR-033]]). The MinIO path has been exercised by hand during Modules 05 and 09, so this is a coverage gap rather than a known defect. Cheapest fix: extend `tests/unit/test_app_boots.py` to construct each adapter under its default setting without performing I/O. | you | **open — cheap, do it in Module 15** |
