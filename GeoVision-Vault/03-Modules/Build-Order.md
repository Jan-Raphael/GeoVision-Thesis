...
| 07 | ◑ **in progress** | 2026-08-28 | | **Code shipped and run for real**: `scripts/split_dataset.py` (grouped-stratified by building, not just site — `dataset/raw/Aldea Grove` turned out to hold three separate houses under one folder name), `ai/data/{transforms,datamodule}.py`, `ai/models/resnet18.py`, `ai/training/{callbacks,trainer,train_classifier}.py`, `ai/configs/train_resnet18.yaml`. A real (non-stub, non-smoke-test) training run completed: 24 epochs, early-stopped, **best validation macro-F1 = 0.4603** — well under the 85% target, honestly attributable to the dataset's size and imbalance (Foundation has only 16 training images after the split) rather than a code defect. `models/classifier/resnet18/v1/best.pt` not yet published (Q10: GitHub Releases / Kaggle). MobileNetV3 comparison model not yet built. |
